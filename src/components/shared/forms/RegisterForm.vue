<script setup lang="ts">
    ...
    const showVerificationNotification = ref<boolean>(false)
    ...
    async function register(): Promise<void>{
        ...
        if (res.items[0]) showVerificationNotification.value = res.items[0];
    }

</script>

<template>
    <div v-if="!showVerificationNotification" class="w-full h-full flex flex-wrap p-8 items-start justify-center">
        ...
        <div class="w-1/2 flex flex-wrap justify-around">
            ...
            <PrimaryButton :text="'Register'" @click="register()" :classes="'w-full h-20'" />
        </div>
    </div>
    <div v-else class="w-full h-full flex flex-wrap p-8 items-center justify-center">
        <h1 class="w-full h-full text-6xl font-bold flex items-center justify-center">An Verification-Mail was sent to {{ account.email }}</h1>
    </div>
</template>
